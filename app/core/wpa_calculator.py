...
class WPACalculator:
    ...
    def calculate_win_probability(self, inning: int, is_top: bool, score_diff: int, outs: int, runners: tuple):
        """
        ...
        if is_top:
            ...
            if runners_score > 0:
                adjustment -= (0.05 * runners_score * (1 + (inning/10))) # Runners hurt Home
            if outs > 0:
                adjustment += (0.02 * outs * (1 + (inning/10))) # Outs help Home
        else:
            # Home team batting
            if runners_score > 0:
                adjustment += (0.05 * runners_score * (1 + (inning/10))) # Runners help Home
            if outs > 0:
...
if __name__ == "__main__":
    ...
    state_start = {
        "inning": 9, "is_top": False, "score_diff": 0, "outs": 0, "runners": (False, False, False)
    }
    
    # 9th Inning, Bottom, Tie Game, 0 outs, Runner on 1st (Hit)
    state_end = {
        "inning": 9, "is_top": False, "score_diff": 0, "outs": 0, "runners": (True, False, False)
    }
