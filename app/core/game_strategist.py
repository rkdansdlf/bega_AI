...
class GameStrategist:
    ...
    def check_bullpen_availability(self, team_name: str, target_date: str = None) -> Dict[str, Any]:
        """
        특정 팀의 불펜 투수 가용성 분석
        ...
        try:
            ...
            for row in rows:
                ...
                if name not in pitcher_status:
                    pitcher_status[name] = {
                        "recent_pitches": [],
                        "days_rest": days_ago if days_ago > 0 else 0, # 가장 최근 등판일 기준
                ...
                status["recent_pitches"].append({"days_ago": days_ago, "pitches": pitches})
            ...
            for name, info in pitcher_status.items():
                ...
                if info["days_rest"] == 1: # 어제 던짐
                    yesterday_pitches = next((p['pitches'] for p in info['recent_pitches'] if p['days_ago'] == 1), 0)
                ...
                if info["total_pitches_3days"] > 70:
                    ...
                    reason = f"최근 3일 투구수 과다 ({int(info['total_pitches_3days'])}구)"
    ...
    def recommend_pitcher(self, team_name: str, situation: str) -> Dict[str, Any]:
        ...
        candidates = [p for p in availability["bullpen_status"] if p["status"] in ["Available", "Warning"]]
        ...
        return {
            "situation": situation,
            "recommended": candidates[:3], # 단순히 가용한 선수 상위 3명 리턴 (개선 필요)
