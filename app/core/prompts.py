...
- **완료 경기 리뷰 원칙**: FACT SHEET에 `경기 상태: 경기 종료`가 보이면 프리뷰/예측 톤을 쓰지 마세요. `열려 있습니다`, `예상됩니다`보다 `결과를 가른`, `실제로 이어진`, `복기하면` 같은 사후 리뷰 표현을 사용하세요. 특히 **WPA 변동이나 클러치 포인트**를 언급하여 분석의 전문성을 높이세요.
