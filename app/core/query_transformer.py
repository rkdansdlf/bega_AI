...
class QueryVariation:
    ...
    variation_type: str           # 변형 유형: 'original', 'expanded', 'statistical', 'contextual'
...
class QueryTransformer:
    ...
    def expand_query_with_rules(self, original_query: str, entity_filter) -> List[QueryVariation]:
        """
        규칙 기반으로 쿼리를 확장합니다.
        ...
        if entity_filter.team_id or entity_filter.player_name:
            context_expansions = self._expand_context_queries(original_query, entity_filter)
            variations.extend(context_expansions)
        
        # 3. 순위/랭킹 쿼리 확장
        if self._is_ranking_query(original_query):
            ranking_expansions = self._expand_ranking_queries(original_query, entity_filter)
    ...
    def _expand_context_queries(self, query: str, entity_filter) -> List[QueryVariation]:
        """팀이나 선수 컨텍스트를 확장합니다."""
        variations = []
        ...
        if entity_filter.team_id:
            ...
            variations.extend([
                ...
                QueryVariation(f"{entity_filter.team_id} 소속 선수 기록", "contextual", 0.7),
            ])
        
        if entity_filter.player_name:
            variations.extend([
                QueryVariation(f"{entity_filter.player_name} 선수 개인 기록", "contextual", 0.8),
                QueryVariation(f"{entity_filter.player_name} 시즌 성적", "contextual", 0.7),
            ])
        
        return variations
    
    def _expand_ranking_queries(self, query: str, entity_filter) -> List[QueryVariation]:
        """순위/랭킹 관련 쿼리를 확장합니다."""
        variations = []
        ...
        for synonym in ranking_synonyms:
            if synonym not in query:
                variations.append(
                    QueryVariation(f"{synonym} {query}", "expanded", 0.6)
                )
    ...
    def _is_ranking_query(self, query: str) -> bool:
        """순위/랭킹 관련 질문인지 판단합니다."""
        ranking_keywords = ["순위", "랭킹", "상위", "1위", "최고", "톱", "TOP", "베스트"]
        ...
        try:
            ...
            for i, line in enumerate(lines[:3]):  # 최대 3개
                if line and len(line) > 10:  # 너무 짧은 응답 제외
                    ...
                    variations.append(QueryVariation(line, variation_type, 0.8 - i*0.1))
            
            logger.info(f"[QueryTransformer] LLM generated {len(variations)} variations")
            return variations
...
async def multi_query_retrieval(
    ...
    """
    logger.info(f"[MultiQuery] Starting retrieval with {len(query_variations)} variations")
    ...
    for variation, task in tasks:
        try:
            ...
            logger.warning(f"[MultiQuery] Failed to retrieve for '{variation.query}': {e}")
    ...
    for doc in all_results:
        ...
        if doc_id not in unique_docs:
            ...
            existing_score = existing.get('similarity', 0) * existing.get('_query_weight', 1)
            ...
            if new_score > existing_score:
                ...
                # 가중 평균으로 점수 결합
                combined_weight = (existing.get('_query_weight', 1) + doc.get('_query_weight', 1)) / 2
                existing['_query_weight'] = combined_weight
    
    # 최종 점수로 정렬 (유사도 * 쿼리 가중치)
    final_docs = list(unique_docs.values())
    final_docs.sort(
        key=lambda d: d.get('similarity', 0) * d.get('_query_weight', 1), 
        reverse=True
    )
    
    logger.info(f"[MultiQuery] Combined {len(all_results)} results into {len(final_docs)} unique docs")
    return final_docs