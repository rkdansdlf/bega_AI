    ...
    def search_documents(self, query: str, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            docs = similarity_search(
                ...
                embeddings[0],
                limit=limit,
                filters={"source_table": "markdown_docs"}, # 설명형 문서는 markdown_docs에 저장됨
                keyword=query
            )
            ...
            if docs:
                ...
                result["found"] = True
                logger.info(f"[DocumentQueryTool] Found {len(docs)} relevant document chunks.")
            else:
                logger.warning(f"[DocumentQueryTool] No relevant documents found for: {query}")
