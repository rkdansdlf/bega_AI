...
현재 **3042 tests / 약 7초** 목표 (모두 오프라인, DB·LLM 호출 없음).

- `test_batch_coach_matchup_cache.py` — lazy import 리팩터로 `--ignore` 불필요
- `test_observability_metrics.py` — `pytest.importorskip` 적용 완료 → prometheus_client 미설치 시 graceful SKIP
