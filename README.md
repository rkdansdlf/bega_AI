...
-   📊 **세이버메트릭스:** ERA-, wRC+, FIP, XR, ISO, BABIP 및 WPA(승리 기여도) 분석
-   ⚡ **SSE 스트리밍:** 실시간 응답 스트리밍
-   🧠 **의도 라우팅:** 통계, 규정, 클러치/WPA 분석 등 의도에 따른 답변 생성
...
-   **의도 라우팅**
    ...
    -   규칙 기반 질문 의도 분류
    -   통계 조회, 순위 질문, 용어 설명, **WPA/클러치 분석** 등 분류
    -   의도별 최적화된 검색 전략
-   **컨텍스트 생성**
    
    -   원본 데이터를 LLM 친화적 텍스트로 변환
    -   세이버메트릭스 지표 동적 계산 (ERA-, wRC+, FIP, XR, ISO, BABIP 등)
    -   **WPA & 결정적 순간** 데이터 추출 및 포맷팅
    -   선수별 순위 및 비교 분석
