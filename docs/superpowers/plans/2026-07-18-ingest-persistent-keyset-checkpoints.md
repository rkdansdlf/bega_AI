...
## Disposable Live PostgreSQL Smoke Verification Evidence

Date: 2026-07-22

- Used an already-local `postgres:16-alpine` image with a disposable `bega_checkpoint_smoke` database bound only to `127.0.0.1:15433`; no shared, production, or external baseball-data source was contacted.
- Applied `003_ai_ingest_orchestration.sql`, applied `004_ai_ingest_checkpoints.sql`, and reapplied 004 with `ON_ERROR_STOP=1`. The second 004 application completed with only expected existing relation/column/index notices.
- A deleted-after-use one-shot harness imported the production `ingest_table`, `build_ingest_lease_guard`, `IngestRunStore`, and `IngestWorker._heartbeat_loop`. It used six clearly synthetic `teams` rows, a minimal local `rag_chunks` table, and `skip_embedding=True`; it made no embedding or external API call.
- The first attempt committed two source rows and checkpoint cursor `SMOKE02`, then stopped before the next source fetch. Resume returned `attempt_source_rows=4`, advanced to `SMOKE06`, accumulated six source rows and six chunks over three committed batches, and marked the checkpoint complete.
- Mutating already committed source row `SMOKE01` before resume did not change its stored chunk, proving the committed prefix was excluded at the resumed source query. A new `SMOKE07` row newer than the persisted source cutoff was also excluded. Re-running the completed checkpoint returned zero attempted source rows and zero attempted chunks.
- A one-second real PostgreSQL lease expired, `recover_expired()` returned `(1, 0)`, and the same run was claimed by `lease-owner-b` with one recovery attempt. A synthetic first-call `psycopg.OperationalError` caused one heartbeat retry; the second call reached the real store successfully without setting lease loss.
- The smoke exited 0 with `CHECKPOINT_POSTGRES_SMOKE_OK`. The disposable container, database, one-shot harness, and generated bytecode were removed immediately afterward.

Residual risk: this checkpoint-focused smoke did not run the pgvector-dependent 001 schema migration. Text keyset comparisons still rely on the source database using the same expression and collation for both the resume predicate and ascending order; the matrix below exercises four available collations.

## Text Keyset Locale/Collation Matrix Verification Evidence

Date: 2026-07-22

- Reused the already-local `postgres:16-alpine` image with a separate disposable `bega_collation_smoke` database bound only to `127.0.0.1:15434`.
- The server exposed `C`, `POSIX`, `und-x-icu`, and `ko-KR-x-icu`. A deleted-after-use one-shot harness imported the production `resolve_checkpoint_order`, `resolve_primary_key_columns`, and `build_select_query` functions.
- Each collation used the same 18 composite keys: nine ASCII/Latin/Greek/Korean text values and integer tie-breakers `1` and `2`. The harness derived the real primary-key order from PostgreSQL instead of supplying a test-only order.
- Every one of the 18 rows served as a persisted-cursor equivalent in each collation. All 72 resume queries excluded the cursor row and returned exactly the suffix of that collation's own ascending full query, including Unicode cursor parameters and equal-text-prefix integer transitions.
- The matrix exited 0 with `KEYSET_COLLATION_MATRIX_OK`. Its disposable container, database, one-shot harness, and generated bytecode were removed immediately afterward.

Residual risk: deployment-specific custom or nondeterministic collations outside these four were not enumerated. Such a deployment must repeat this matrix against its exact source-column collation. The pgvector-dependent 001 schema migration also remains outside this checkpoint-focused verification.

## Same-Field Cutoff and Watermark Verification Evidence

Date: 2026-07-18

- Focused behavior RED `/Users/mac/project/KBO_platform/bega_AI/.venv/bin/python -m pytest tests/test_ingest_query.py tests/test_ingest_checkpoint_integration.py -q -k 'full_cutoff_eligibility_is_exact or cutoffs_match_exclusive_watermark or reference_profile_full_checkpoint or reference_profile_cutoff_blocks or game_configured_watermark_does_not_fall or game_full_null_metadata_update'` exited 1 before production changes with 2 passed, 8 failed, 81 deselected, and 5 existing `datetime.utcnow()` warnings. A tightened concurrent-update sub-selection independently exited 1 with 2 failed and 41 deselected because both `teams` and `stadiums` admitted the post-cutoff suffix.
- Focused behavior GREEN: the same selection exited 0 with 10 passed, 0 failed, 81 deselected, and 5 existing warnings. The combined checkpoint/query/integration/documentation regression exited 0 with 201 passed, 0 failed, 0 skipped, and 33 existing warnings.
- Documentation contract RED `/Users/mac/project/KBO_platform/bega_AI/.venv/bin/python -m pytest tests/test_ai_schema_migrations.py -q -k 'runbook_documents_persistent or design_documents_timestamp'` exited 1 with 2 failed and 5 deselected before the design and runbook corrections; the same command then exited 0 with 2 passed and 5 deselected.
- Expanded ingest/schema/worker regression `/Users/mac/project/KBO_platform/bega_AI/.venv/bin/python -m pytest tests/test_ingest_checkpoints.py tests/test_ingest_checkpoint_integration.py tests/test_ingest_query.py tests/test_ingest_results.py tests/test_ingest_worker.py tests/test_ingest_run_store.py tests/test_ingest_runs.py tests/test_ingest_router.py tests/test_ingest_parallel_engine.py tests/test_observability_metrics.py tests/test_ai_schema_migrations.py tests/test_ai_schema_rehearsal_script.py tests/test_db_schema_contract.py tests/test_rag_storage_schema.py tests/test_schema_startup_mode.py tests/test_validate_ai_runtime_schema.py -q` exited 0 with 322 passed, 0 failed, 0 skipped, and 34 existing deprecation warnings.
- Runtime, tests, and behavior documentation were committed as `4daaa18` (`fix: align checkpoint cutoffs with watermarks`). All 23 trusted database profiles now explicitly declare their projected watermark field or an empty field set. `teams` and `stadiums` bind both FULL cutoff filtering and watermark extraction to generic `updated_at`; configured watermark fields are exclusive; and a FULL `game` row with NULL `game_updated_at` remains ingestable without advancing from `g.updated_at`.
- Static compile `/Users/mac/project/KBO_platform/bega_AI/.venv/bin/python -m compileall -q app scripts tests` exited 0 with no output.
- Baseball data policy `python3 /Users/mac/project/KBO_platform/scripts/validate_baseball_data_policy.py` exited 0: `External baseball data policy OK`.
- OpenAPI contract `/Users/mac/project/KBO_platform/bega_AI/.venv/bin/python scripts/export_openapi_contract.py --check` exited 0: `AI OpenAPI artifacts are current`; one existing `google.generativeai` FutureWarning.
- Full AI suite `/Users/mac/project/KBO_platform/bega_AI/.venv/bin/python -m pytest tests/ -q` exited 0 with 1942 passed, 5 skipped, 0 failed, and 41 warnings in 74.48 seconds. The skips require unavailable local operator-data migration, validation, or handoff artifacts; warnings are existing dependency/API deprecations.
- Committed whitespace gate `git diff --check HEAD^ HEAD` exited 0 with no output. `git ls-files .superpowers/sdd` produced no output before this verification addendum.
