...
Residual risk: this checkpoint-focused smoke did not run the pgvector-dependent 001 schema migration or a locale/collation matrix. Text keyset comparisons therefore still rely on the source database using the same expression and collation for both the resume predicate and ascending order.
